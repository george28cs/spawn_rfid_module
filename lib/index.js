...
function InputEvent(device, options) {
  ...
  this.device = device;
  this.options = options || { encoding: null };
  //this.options.fd = fs.openSync(this.device, this.options.flags);
  this.deviceReader = spawn('cat', [this.device], this.options);

  this.deviceReader.stderr.on('data', (data) => {
    self.emit('error', data.toString());
  });

  this.fd = this.deviceReader.stdout;
...
InputEvent.prototype.destroy = function (callback) {
  this.fd.destroy(callback);
};

InputEvent.prototype.getPid = function () {
  return this.deviceReader.pid
};
/**
 * [EVENT_TYPES]
 */
